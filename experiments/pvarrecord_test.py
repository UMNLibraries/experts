...
from pyrsistent import (
    ...
    field, InvariantException, ny, pset, PSet, CheckedPVector,
    PTypeError, pset_field, pvector_field, pmap_field, pmap, PMap,
    pvector, PVector, v, m)

from _pvarrecord import PVarRecord as PRecord
...
def test_create_ignore_extra_true():
    ...
    assert h
    assert h.point1.extra_field_0 == 'extra_data_0'
    assert h.point2.extra_field_1 == 'extra_data_1'
    assert h.extra_field_2 == 'extra_data_2'
...
def test_create_ignore_extra_true_sequence_hierarchy():
    ...
    assert h
    assert h.points[0].extra_field_2 == 'extra_data_2'
    assert h.points[1].extra_field_3 == 'extra_data_3'
    assert h.extra_field____ == 'extra_data_2'
...
def test_ignore_extra_for_pvector_field():
    ...
    h = HierarchyA.create({'points': [point_object]})
    assert h
    assert h.points[0].extra_field == 69
    #h = HierarchyB.create({'points': [point_object]}, ignore_extra=True)
    h = HierarchyB.create({'points': [point_object]})
    assert h
    assert h.points[0].extra_field == 69
...
def test_create_allow_extra():
    #r = ARecord.create({'x': 1, 'y': 'foo', 'z': None}, ignore_extra=True)
    r = ARecord.create({'x': 1, 'y': 'foo', 'z': None})
    assert r.x == 1
    assert r.y == 'foo'
    assert r.z is None
    assert isinstance(r, ARecord)
...
def test_correct_assignment_of_constructed_extra_field():
    r = ARecord(x=1, y='foo', z=200)
    r2 = r.set('z', 300)

    assert r == {'x': 1, 'y': 'foo', 'z': 200}
    assert r2 == {'x': 1, 'y': 'foo','z': 300}
    assert isinstance(r2, ARecord)

def test_correct_assignment_of_unconstructed_extra_field():
    r = ARecord(x=1, y='foo')
    r2 = r.set('z', 300)
...
def test_pset_field_optional():
    ...
    """
    class Record(PRecord):
        # TODO: Don't think I want to allow optional values to be None, unless None is explicitly allowed
...
def test_pset_field_name_string_type():
    ...
    """
    class Record(PRecord):
        value = pset_field("pvarrecord_test.Something")
...
def test_pset_multiple_string_types_field_name():
    ...
    """
    class Record(PRecord):
        value = pset_field(("pvarrecord_test.Something", "pvarrecord_test.Another"))
...
def test_pvector_field_name_string_type():
    ...
    """
    class Record(PRecord):
        value = pvector_field("pvarrecord_test.Something")
...
def test_pvector_multiple_string_types_field_name():
    ...
    """
    class Record(PRecord):
        value = pvector_field(("pvarrecord_test.Something", "pvarrecord_test.Another"))
...
def test_pmap_field_name_string_type():
    ...
    """
    class Record(PRecord):
        value = pmap_field("pvarrecord_test.Something", "pvarrecord_test.Another")
...
def test_pmap_field_name_multiple_string_types():
    ...
    """
    class Record(PRecord):
        value = pmap_field(("pvarrecord_test.Something", "pvarrecord_test.Another"), int)
        value2 = pmap_field(str, ("pvarrecord_test.Something", "pvarrecord_test.Another"))
    assert ((Record().value.__class__.__name__,
             Record().value2.__class__.__name__) ==
