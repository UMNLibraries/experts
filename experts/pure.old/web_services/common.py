...
'''Parent path of swagger.io/json schema files for Pure Web Services versions. Child
...
'''All Pure Web Services versions this package recognizes and supports.'''
...
'''Latest valid Pure Web Services version.'''

oldest_version: str = str(min([int(version) for version in versions]))
'''Oldest valid Pure Web Services version.'''

env_version_varname: str = 'PURE_WEB_SERVICES_VERSION'
'''Environment variable name for a Pure Web Services version. Defaults to
PURE_WEB_SERVICES_VERSION.  Used by ``env_version()``.
'''
...
class PureWebServicesMissingVersionError(ValueError, PureWebServicesException):
    '''Raised when a Pure Web Services version is expected but missing.'''
    def __init__(self, *args, **kwargs):
        super().__init__(f'No version found in kwargs, default_version, or {env_version_varname}', *args, **kwargs)

class PureWebServicesInvalidVersionError(ValueError, PureWebServicesException):
    '''Raised when a Pure Web Services version is unrecognized.'''
...
def validate_version(func: F) -> F:
    '''A decorator wrapper that validates a kwarg Pure Web Services version.
    ...
    Raises:
        PureWebServicesMissingVersionError: If the ``version`` kwarg is missing or
            the value is None.
        PureWebServicesInvalidVersionError: If the ``version`` is unrecognized.
    '''
    ...
    def wrapper_validate_version(*args, **kwargs):
        if 'version' in kwargs and kwargs['version'] is not None:
            if not valid_version(kwargs['version']):
                raise PureWebServicesInvalidVersionError(kwargs['version'])
        else:
            kwargs['version'] = None
        if kwargs['version'] is None and default_version() is not None:
            if valid_version(default_version()):
                ...
                raise PureWebServicesInvalidVersionError(default_version())
        if kwargs['version'] is None:
            raise PureWebServicesMissingVersionError()
        return func(*args, **kwargs)
    return cast(F, wrapper_validate_version)

def schema_524() -> MutableMapping:
    '''Returns a mapping representation of the Pure Web Services version 5.24 schema.'''
    with open(schemas_path  / '524/swagger.json') as json_file:
        return json.load(json_file)

def schema_523() -> MutableMapping:
    '''Returns a mapping representation of the Pure Web Services version 5.23 schema.'''
...
def schema_for(*, version: str = None) -> MutableMapping:
    '''Returns a mapping representation of the schema for the given Pure Web Services
    ``version``.'''
    return globals()[f'schema_{version}']()

def collections_524() -> Tuple[str]:
    '''Returns a tuple of all collection names in the Pure Web Services version 5.24
    schema.'''
    return tuple(map(lambda tag: tag['name'], schema_for(version='524')['tags']))

def collections_523() -> Tuple[str]:
    '''Returns a tuple of all collection names in the Pure Web Services version 5.23
    schema.'''
...
class PureWebServicesInvalidCollectionError(ValueError, PureWebServicesException):
...
def validate_collection(func: F) -> F:
    '''A decorator wrapper that validates a kwarg Pure Web Services ``collection`` for a
    kwarg ``version``.
    ...
    Raises:
        PureWebServicesInvalidCollectionError: If the given ``collection`` is invalid
        for the ``version``.
    '''
    @functools.wraps(func)
    @validate_version
    def wrapper_validate_collection(*args, **kwargs):
        if not valid_collection(collection=kwargs['collection'], version=kwargs['version']):
            raise PureWebServicesInvalidCollectionError(collection=kwargs['collection'], version=kwargs['version'])
        return func(*args, **kwargs)
