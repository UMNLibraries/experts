...
from experts.pure.web_services import response
from experts.pure.web_services.common import default_version, valid_collection, valid_version, PureWebServicesInvalidCollectionError, PureWebServicesInvalidVersionError
from experts.pure.web_services.exceptions import PureWebServicesException

env_key_varname: str = 'PURE_WEB_SERVICES_KEY'
'''Environment variable name for a Pure Web Services key. Defaults to PURE_WEB_SERVICES_KEY.
Used by ``env_key()``.
'''
...
def default_retryer() -> Callable:
    '''A function that retries HTTP requests to the Pure Web Services server.
...
class PureWebServicesClientException(PureWebServicesException):
    '''Base class for exceptions specific to pure.web_services.client.'''
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

env_domain_varname: str = 'PURE_WEB_SERVICES_DOMAIN'
'''Environment variable name for a Pure Web Services domain. Defaults to PURE_WEB_SERVICES_DOMAIN.
Used by env_domain().
'''
...
def _get_collection_from_resource_path(resource_path: str, version: str) -> str:
    '''Extracts the collection name from a Pure Web Services URL resource path.

    Args:
        resource_path: URL path, without the base URL, to a Pure Web Services resource.
        config: Instance of pure.web_services.client.Config.

    Returns:
        The name of the collection.

    Raises:
        common.PureWebServicesInvalidCollectionError: If the extracted collection is
            invalid for the given Web Services version.
    '''
    collection = resource_path.split('/')[0]
    if not valid_collection(collection=collection, version=version):
        raise PureWebServicesInvalidCollectionError(collection=collection, version=version)
    return collection

class PureWebServicesHTTPError(HTTPError, PureWebServicesClientException):
    '''Raised in case of an HTTP error response.'''
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

class PureWebServicesRequestException(RequestException, PureWebServicesClientException):
...
class Config:
    ...
    Examples:
        >>> from experts.pure.web_services import client
    ...
    '''Domain of a Pure Web Services server. Required. Default: Return value of ``env_domain()``.'''
    ...
    '''Base path of the Pure Web Services URL entry point, without the version number segment.
    ...
    '''Pure Web Services version, without the decimal point. For example, ``517`` for version 5.17.
    Default: Return value of ``default_version()``.'''
    @version.validator
    def validate_version(self, attribute: str, value: str) -> None:
        if not valid_version(value):
            raise PureWebServicesInvalidVersionError(value)

    key: str = attr.ib(
        factory=env_key,
        validator=attr.validators.instance_of(str)
    )
    '''Pure Web Services key. Required. Default: Return value of ``env_key()``.'''
    ...
    '''A function that retries HTTP requests to the Pure Web Services server. Must have
    ...
    '''Pure Web Services entrypoint URL. Should not be included in constructor
...
def get(resource_path: str, params: Mapping = None, config: Config = None) -> requests.Response:
    '''Makes an HTTP GET request for Pure Web Services resources.
    ...
    Args:
        resource_path: URL path to a Pure Web Services resource, to be appended to the
    ...
    Raises:
        common.PureWebServicesInvalidCollectionError: If the collection, the first
            segment in the resource_path, is invalid for the given Web Services version.
        PureWebServicesHTTPError: If the response includes an HTTP error code, possibly
            after multiple retries.
        PureWebServicesRequestException: If the request generated some error unrelated
            to any HTTP error status.
        PureWebServicesClientException: Some unexpected exception that is none of the
            above.
    '''
    if params is None:
        params = {}
    ...
    with requests.Session() as s:
        ...
        try:
            ...
            raise PureWebServicesHTTPError(
            ...
            raise PureWebServicesRequestException(
            ...
            raise PureWebServicesClientException(
...
def get_all_changes(start_date: str, params: Mapping = None, config: Config = None) -> Iterator[requests.Response]:
    ...
    records per request. Though the Pure Web Services documentation includes support
    for that parameter, it seems to be ignored. The Pure Web Services may actually
...
def filter(resource_path: str, payload: Mapping = None, config: Config = None) -> requests.Response:
    '''Makes an HTTP POST request for Pure Web Services resources, filtered according to
        the ``payload``.
