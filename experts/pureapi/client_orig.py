...
@safe
def attempt_request(session, prepped, timeout: int=60):
    return session.send(prepped, timeout=timeout)

def retryable(result):
    http_5xx_status = re.compile('^5\d\d')
    if is_successful(result):
        response = result.unwrap()
        if (http_5xx_status.match(str(response.status_code)) or
       ...
       exc = result.failure()
       if isinstance(e, (ConnectionError, ConnectTimeout, ReadTimeout)):
           return True
       else:
           return False

def manage_request_attempts(session, prepped, attempts_id: str, timeout: int=60, wait_seconds: int=2, max_attempts: int=10, attempt_number: int=1):
    ...
    result = attempt_request(session, prepped, timeout=timeout)
    ...
    if retryable(result) and attempt_number < max_attempts:
        ...
        return manage_request_attempts(session, prepped, attempts_id, timeout, wait_seconds**2, max_attempts, attempt_number=attempt_number+1)
    else:
        return result       
