...
# We expect the following PMaps to have these key-value pairs:
# size: int
# offset: int
PureRequestPageParams = PMap
...
class PureRequestPageParamsParser:
    ...
    def update_offset(params:PureRequestPageParams, new_offset:int) -> PureRequestPageParams:
        # This returns a new PureRequestPageParams:
...
# The following seems likely to lead to circular dependencies, because the types are too specific:
#ResponsePage_contra = TypeVar('ResponsePage_contra', PureResponsePage, ScopusResponsePage, contravariant=True)

# The following causes mypy errors like the following: 
# Argument 1 to "parse_response" has incompatible type "type[ScopusResponsePageParser]"; expected "ResponsePageParser"  [arg-type]
#Following member(s) of "ScopusResponsePageParser" have conflicts:
#    Expected:
#        def count(response: Mapping[Any, Any]) -> int
#    Got:
#        def count(response: ScopusResponsePage) -> int
#    Expected:
#        def items(response: Mapping[Any, Any]) -> Iterator[Mapping[Any, Any]]
#    Got:
#        def items(response: ScopusResponsePage) -> Iterator[Mapping[Any, Any]]
...
def parse_response(parser:ResponsePageParser, response:ResponsePage):
