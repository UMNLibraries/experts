...
from experts.pure.web_services import client, common

from tests.pure.web_services import changes_including_zero_counts
...
def test_config():
    ...
    with pytest.raises(common.PureWebServicesInvalidVersionError):
...
def test_get_collection_from_resource_path():
    ...
    with pytest.raises(common.PureWebServicesInvalidCollectionError):
...
def test_get_exception(version):
    ...
    with pytest.raises(client.PureWebServicesHTTPError, match='404') as exc_info:
...
def test_filter_exception(version):
    config = client.Config(version=version)
    #with pytest.raises(client.PureWebServicesHTTPError, match='404') as exc_info:
    with pytest.raises(client.PureWebServicesHTTPError, match='500') as exc_info:
        payload = {
            "size": 1,
            "offset": 0,
            "forOrganisationalUnits": {
