...
from experts.pure.web_services.common import PureWebServicesInvalidCollectionError, PureWebServicesInvalidVersionError

uuid_regex = re.compile('^[a-z0-9-]+$')

def load_json(version, record_type, uuid):
    with open(f'tests/pure/web_services/data/{version}/{record_type}/{uuid}.json') as f:
        return json.load(f)
...
def test_transformer_for(version):
    ...
    with pytest.raises(PureWebServicesInvalidCollectionError):
        response.transformer_for(
            collection='bogus',
            version=version
        )

    with pytest.raises(PureWebServicesInvalidVersionError):
        response.transformer_for(
            collection='persons',
            version='bogus'
        )
...
def test_person(version):
    ...
    p3 = transformer(load_json(version, 'person', jan_fransen_uuid))
...
def test_external_person(version):
    ...
    p3 = transformer(load_json(version, 'external_person', sambrano_uuid))
    assert isinstance(p3.uuid, str)
...
def test_organisational_unit(version):
    ...
    ou2 = transformer(load_json(version, 'organisational_unit', libraries_uuid))
    ...
    ou3 = transformer(load_json(version, 'organisational_unit', clawriting_uuid))
...
def test_external_organisation(version):
    ...
    eo2 = transformer(load_json(version, 'external_organisation', rutherford_lab_uuid))
...
def test_research_output(version):
    ...
    ro3 = transformer(load_json(version, 'research_output', cabbibo_decays_uuid))
