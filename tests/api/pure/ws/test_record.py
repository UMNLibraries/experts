...
from experts.api.pure.ws import Record

api_version = '524'

def load_record(collection:str, uuid:str) -> dict:
    with open(f'tests/api/pure/ws/data/{api_version}/{collection}/{uuid}.json') as record_file:
        return json.load(record_file)

def test_record():
    record_dict = load_record(
        collection = 'person',
        uuid = '01edf3d8-7e44-4dfa-bec4-8e3472965e1f',
    )

    # The constructor validates the types returned by the special property methods,
    # so this is the only test we need:
