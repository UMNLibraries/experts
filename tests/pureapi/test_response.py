...
from experts.pureapi.common import PureAPIInvalidCollectionError, PureAPIInvalidVersionError

uuid_regex = re.compile('^[a-z0-9-]+$')

def load_json(version, record_type, uuid):
    with open(f'tests/pureapi/data/{version}/{record_type}/{uuid}.json') as f:
        return json.load(f)
